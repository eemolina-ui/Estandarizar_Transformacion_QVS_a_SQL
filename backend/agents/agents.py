...
from templates.prompts_str import generic_prompt_str, rag_prompt_str, classifier_prompt_str, waving_prompt_string, prompt_avatar_str
...
class MultiAgentsBot:
    def __init__(self, vdb_client=None, data_analyst = None):
        ...
        self.avatar_prompt =        PromptTemplate(input_variables=["human_input"],
                                                   template=prompt_avatar_str)
        ...
        self.avatar_chain =         self.avatar_prompt | self.generic_llm
    ...
    def query_avatar(self, human_input):
        return self.avatar_chain.invoke({'human_input': human_input}).content